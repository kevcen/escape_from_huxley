...
gameover_font = pg.font.SysFont("papyrus", 100)
...
def draw_sprites():
    ...
    plyr.fall(DISPLAY_SIZE[0], GRAVITY)
    plyr.draw(display)
    draw_tiles()
...
def gameover():
    ...
    gameover_text = gameover_font.render("GAME OVER", 1, col.RED.value)
    display.blit(gameover_text, ((DISPLAY_SIZE[0] -
                                  gameover_text.get_width()) // 2,
                                 (DISPLAY_SIZE[1] - gameover_text.get_height())
                                 // 2))
    pg.display.update()
    pg.time.delay(1500)
...
def game_loop():
    ...
    while run:
        ...
        # draw the spritespg.font.SysFont("papyrus", 15)
        draw_sprites()
        move_bullets()
